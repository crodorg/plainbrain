...
2. **Gitignore + activate.** Ensure `.claude/state/` is ignored; `git rm -r --cached` any
   ...
   un-adopted repos are untouched. Include an `enforce: on` line — the gates (driver-read
   before edits, why-before-plan-rewrite, plan scope) are DEFAULT-ON for new adoptions;
   omit it only if the user explicitly wants them off (see the README's Gates section).
